...
def parse_args():
    ...
    parser.add_argument('--wd', default=4e-3, type=float, help='weight decay')
    parser.add_argument('--seed', default=2020, type=int, help='random seed to set')
    parser.add_argument('--data_root', default='C:\\Users\\lenovo\\dataset\\cifar', type=str, help='dataset path')
    # resume
    ...
    # network
    parser.add_argument('--action',type=str, help='pruning preserve ratio')

    return parser.parse_args()


# def get_model(select):
...
def train(epoch, train_loader):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        print(batch_idx, len(train_loader), 'Loss: {:.3f} | Acc1: {:.3f}% | Acc5: {:.3f}%'
...
def test(epoch, test_loader, save=True):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            print(batch_idx, len(test_loader), 'Loss: {:.3f} | Acc1: {:.3f}% | Acc5: {:.3f}%'
                         .format(losses.avg, top1.avg, top5.avg))
...
if __name__ == '__main__':
    ...
    action = [float(bi) for bi in action]
    ...
    print(action)
    net = pruner.get_masked(select=action)# for measure
    ...
    del net
    net = pruner.get_masked(select=action)  # real training
